...
class AuthViewModel extends GetxController {
  ...
  void onInit() {
    ...
    if (savedUser != null) {
      ...
      if (StorageService.to.getToken() != null && savedUser.role != 'Admin') {
        fetchUserProfile();
      }
    }
  }
  ...
  Future<void> fetchUserProfile() async {
    try {
      ...
      if (response != null) {
        final token = user.value.token ?? StorageService.to.getToken();
      ...
      if (e.toString().contains("BannedUser")) {
        ...
        Get.snackbar(
          "Banned",
          "Banned user. Please contact to admin",
  ...
  Future<void> login(String email, String password) async {
    ...
    try {
      ...
      if (isRememberMe.value) {
        await StorageService.to.setSavedEmail(email);
        await StorageService.to.setSavedPassword(password);
      } else {
