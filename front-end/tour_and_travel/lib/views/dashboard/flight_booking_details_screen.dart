  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      final dynamic booking = bookingViewModel.bookingHistory.firstWhere(
        (b) => (b['referenceId'] ?? b['id'] ?? 0) == bookingId && (b['type'] ?? 'Flight').toString().toLowerCase() == bookingType.toLowerCase(),
        orElse: () => this.booking,
      );
                  ...
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 20),
                        _buildDetailRow("Cabin Class", booking['cabinClass'] ?? "Economy"),
                        _buildDetailRow("Airline Name", booking['airline'] ?? "Emirates"),
                        _buildDetailRow("Flight Number", "FL-${booking['itemId']?.toString().padLeft(4, '0') ?? '0613'}"),
                        ...
                        _buildDetailRow("Traveler", booking['userName'] ?? "Traveler"),
                      ],
                    ),
                  ),
                ],
              ...
              child: ElevatedButton.icon(
                onPressed: () async {
                  ...
                  await Printing.sharePdf(
                    bytes: pdfBytes,
                    filename: 'Invoice_Flight_${DateTime.now().millisecondsSinceEpoch}.pdf',
                  );
                },
